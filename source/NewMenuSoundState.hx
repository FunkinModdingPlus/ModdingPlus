...
class NewMenuSoundState extends MusicBeatState
{
	var addCharUi:FlxUI;
	...
	override function create()
	{
		...
		add(bg);
		mainPngButton = new FlxButton(10,10,"Confirm Sound Files",function ():Void {
		...
		likeText = new FlxUIInputText(100, 10, 70,"customMenuConfirm");
	...
	function writeCharacters() {
		...
		#if sys
		if (!FileSystem.exists('assets/sounds/custom_menu_sounds/'+nameText.text)) {
			FileSystem.createDirectory('assets/sounds/custom_menu_sounds/'+nameText.text);
		}

		for (epicFile in epicFiles) {
			var coolPath:Path = new Path(epicFile);
			coolPath.dir = 'assets/sounds/custom_menu_sounds/'+nameText.text;
			var pathString:String = coolPath.dir + '/' + coolPath.file + '.' + coolPath.ext;
			File.copy(epicFile,pathString);
		}

		var epicCustomMenuSoundsFile:Dynamic =CoolUtil.parseJson(Assets.getText('assets/sounds/custom_menu_sounds/custom_menu_sounds.json'));
		trace("parsed");
		Reflect.setField(epicCustomMenuSoundsFile,likeText.text,nameText.text);
